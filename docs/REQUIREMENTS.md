...
- 사용자 로그인/session/bearer token 검증과 viewer/operator/approver/admin 역할 기반 읽기/scan
  실행/exception 승인 권한을 처리하는 Dashboard/API 인증·인가 boundary
  ([ARCHITECTURE.md](./ARCHITECTURE.md) §Dashboard/API backend and auth boundary)
