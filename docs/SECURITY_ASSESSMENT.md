...
이 문서는 고객사 인프라 적용 전 내부 최종점검 환경에서 실제 납품 대상 산출물과 개발 환경 cluster에 적용된 설정을 기준으로 보안 위험을 점검하기 위한 실행 환경, 입력 산출물, 대시보드 메뉴, 판정 정책을 정의한다.

## Scope

현재 버전의 최종점검은 납품 산출물과 개발 환경 cluster에 실제 적용된 Kubernetes 설정을 기준으로 수행한다. 실시간 런타임 이벤트 탐지와 산출물-런타임 drift 분석은 다음 버전 범위로 둔다.
...
| Applied Cluster Configuration Assessment | 포함 | 개발 cluster에 적용된 Workload YAML, RBAC, ServiceAccount, Secret 참조, securityContext, volume 설정 |
| Runtime Event/Drift Assessment | Next Version | Falco/Tetragon runtime event, 산출물-클러스터 drift, 행위 기반 탐지 |
...
| Cluster access | 개발 환경 cluster read-only kubeconfig | 적용된 YAML, RBAC, Secret 참조 방식 검수 |
...
| Trivy Operator | 개발 cluster에 적용된 workload image 취약점 확인 | 실시간 runtime 탐지가 아니라 applied cluster 검수 보조로 사용 |
...
| Dev cluster kubeconfig | 필요 시 | read-only 권한으로 적용된 리소스 검수 |

## Cluster Access Policy

개발 환경 cluster 접근은 현재 버전에 포함하되, 실시간 런타임 탐지 목적이 아니라 적용된 설정 검수 목적으로 제한한다.
...
| Kubernetes Config & RBAC | 배포 산출물과 개발 cluster 적용 설정의 권한이 과도한가? | privileged, hostPath, hostNetwork, capabilities, Secret references, RBAC wildcard, cluster-admin | manifest와 applied resource 비교 확인 |
...
| Runtime event | Falco/Tetragon 이벤트 기반 고위험 행위 확인 |
