...
단건.
list item 스키마에 `summary.final_decision_detail`(`{status, reasons[]}`) 필드를 추가한 스키마.
...
```json
{
  ...
  "summary": {
    ...
    "final_decision_detail": {
      "status": "Fail",
      "reasons": [
        "Critical finding without approved exception",
        "scan_health Fail exists"
      ]
    }
  },
  "created_at": "2026-06-18T11:00:00Z",
...
feature ID 매핑이다.
backend는 enum/참조 값 검증만 하고, 실제 병합·resolve와 unknown feature(`features[].name`)의 `ConfigError` 처리는 operator가 deterministic하게 수행한다(unknown `profiles[]`는 admission에서 이미 거부되어 이 단계에 도달하지 않는다).
