...
| P7 | S3 | M5 | Trivy delivery image report integration, image integrity, optional VulnerabilityReport ingestion |
...
- Semgrep/gosec, Gitleaks, Trivy/Grype, Syft, Cosign/Notation, Crane,
  kube-linter, conftest, Hadolint, ShellCheck scanner config와 실행 wrapper.
...
- scripts/run-security-assessment.sh가 artifact input을 읽고 선택된 scanner를 실행해
  raw report artifact를 생성.
- 승인 digest 비교를 위한 scripts/verify-image-digest.sh.
- scanner 결과 정규화용 scripts/normalize-findings.sh가 최소 공통 finding JSONL과
  scan health JSON을 생성.
...
않는다. M0.5는 placeholder가 아니라 실제 납품 산출물 입력을 기준으로 scanner
raw report를 생성하는 baseline이다. M5는 Trivy/image integrity 기능을 controller
feature로 통합하고, optional VulnerabilityReport ingestion과 duplicate-safe finding
aggregation을 구현하는 단계로 둔다.
...
- source, secret, image, SBOM/무결성, manifest/RBAC, Dockerfile/script 경로 중
  입력이 제공된 항목은 non-empty raw report artifact를 생성.
- scanner binary가 없거나 registry/image 접근이 실패하면 pass가 아니라
  scan health `Error` 또는 `Fail`로 기록.
- normalize-findings.sh가 raw report를 stable path의 normalized JSONL과
  scan-health JSON으로 변환.
...
- M0.5에서 생성한 raw scanner report와 minimal normalized finding fixture를
  primary input으로 사용하는 ingestion path.
...
- M0.5 raw report fixture가 Artifact Store path와 Metadata Store index로 적재됨.
...
## P5 - M3 Security Assessment feature integration

```text
docs/ROADMAP.md의 M3, Security Assessment feature integration을 구현한다.

범위:

- security_assessment feature config default와 validation.
- delivery artifact scan을 위한 Assessment Job/CronJob resource.
- M0.5 scanner runner, scanner config, artifact input manifest, report output
  convention을 Job/CronJob에 연결.
- M1 Security Finding Schema와 Report Store interface를 사용하는 finding
...
optional inventory, Trivy controller feature integration, applied cluster configuration
scan은 아직 구현하지 않는다. M3는 scanner binary별 실행 로직을 새로 만들지 않고
M0.5 runner/report convention을 controller-managed resource로 실행 가능하게
연결하는 단계다.
...
- security_assessment feature 비활성화 시 stale run-scoped resource가 GC 대상
  으로 제거되거나 표시됨.
- Assessment Job/CronJob이 M0.5 artifact input과 report output path convention을 사용.
...
## P7 - M5 Trivy delivery image integration and integrity

```text
docs/ROADMAP.md의 M5, Trivy delivery image report 경로와 image integrity를
controller feature로 통합한다.

범위:

- delivery image scanning을 위한 trivy feature config default와 validation.
- M0.5에서 만든 registry digest 또는 image tar scan report 경로를
  Feature/Report Store workflow에 연결.
- Syft 또는 Trivy SBOM output을 사용하는 SBOM artifact ingestion.
- Crane과 승인 digest list를 사용하는 digest verification artifact ingestion.
...
M5는 M0.5 scanner runner를 재설계하지 않고 image/SBOM/digest artifact ingestion,
deduplication, optional VulnerabilityReport merge semantics만 담당한다.
