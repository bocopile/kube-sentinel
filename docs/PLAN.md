...
|----------|------|----------|------|
| Code / Artifact Workflow | source, Dockerfile, Helm/YAML, RBAC manifest, script, image list, digest list, SBOM | 산출물 입력 검증, scanner 실행, PostgreSQL `raw_reports` 저장, finding 정규화 | `ScanRun.status.artifactScan` |
