...
Reconcile()
  ...
  ├── 8. BuildActiveFeatures() → 우선순위 순서로 Feature 목록 구성
  ├── 9. 각 Feature.Contribute() → Mgmt-local/Biz-remote DesiredStateStore에 리소스 기여
  ├── 10. OTelConfig() 수집 → OTel Node/Gateway ConfigMap 자동 합성
  ├── 11. Override 적용 (공통 nodeAgent → 도구별 순서)
  ├── 12. Mgmt Cluster 리소스 SSA Apply
  ├── 13. Biz Cluster Scan 실행
  │      ├── read-only applied config inspection
  │      └── 필요 시 허용된 scanner Job Remote SSA Apply
  ├── 14. label 기반 비활성 Feature/ScanRun remote GC
  ├── 15. Code 결과와 Biz Cluster 결과 상관 분석
