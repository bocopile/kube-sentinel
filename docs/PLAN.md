...
`<scanner>`는 정규화 값이며 Trivy CLI 직접 scan과 Trivy Operator `VulnerabilityReport`는 모두 `trivy`, Grype는 `grype`다. M5 통과 조건은 같은 report를 2회 처리하거나 두 Trivy 입력 경로가 같은 이미지·CVE·패키지를 보고해도 동일 finding id 기준으로 PostgreSQL `findings` 집계가 중복 증가하지 않는 것이다. 최신 상태 판정은 finding id와 scan timestamp 기준의 PostgreSQL query에서 수행하며, report artifact는 해당 시점의 immutable export로만 사용한다.
...
독립 재실행 트리거는 선언형 `ScanRun.spec`이 아니라 `PATCH /api/v1/scan-runs/{id}/retry` 액션이다. backend는 ScanRun `metadata.annotations`에 `security.kube-sentinel.io/retry-scope`(+ request id/requestedAt)를 patch하고, reconciler는 이 annotation을 보고 선택 phase만 `Pending`으로 되돌려 재실행한 뒤 annotation을 observed 처리한다. scope별 동작:
...
부분 재실행으로 생성된 finding은 동일 finding_id 기준으로 PostgreSQL `findings`에 멱등 upsert되어 중복 집계되지 않는다(I-8 규칙·M5 dedup).
