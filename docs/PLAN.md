...
> **파이프라인**: Scanner / Read-only Inspector → Finding Normalizer → Report Store → Dashboard
> **보안 점검**: SAST · Secret Scan · Image Scan · SBOM/무결성 · Manifest/RBAC/Dockerfile/Script Scan
> **아키텍처**: Mgmt Cluster 원격 apply + Assessment Workflow Registry
...
> **비목표**: Biz Cluster operator 설치 · 인프라 자동 개선 · 인라인 차단 · Kafka · 완벽한 OCSF 정규화 · 운영 차단 정책
...
flowchart LR
    ...
    operator --> registry
    registry --> store
    store -->|"Apply scan resources"| workloads
    ...
    finding_schema --> report_store["Report Store"]
...
- Report Store는 raw report, normalized finding, scan health, final decision, evidence bundle의 기준 저장소로 사용한다.
...
Priority 200  trivy                ← 이미지 CVE/SBOM/digest 및 optional VulnerabilityReport 정규화
```
...
Reconcile()
  ...
  ├── 3. securityAssessment.scope 검증 → Code / Artifact Scan 범위 확정
  ├── 4. Code / Artifact Scan 실행 또는 Job remote/local apply
  ├── 5. artifact report, image report, manifest report 정규화
  ├── 6. ClusterTarget.spec.kubeconfigRef Secret 조회
  ├── 7. Biz Cluster preflight
  │      ├── API server 연결 확인
  │      ├── namespace allowlist 확인
  │      ├── read-only RBAC 확인
  │      └── optional CRD/bootstrap capability 확인
  ├── 8. BuildActiveFeatures() → 우선순위 순서로 검사 기능 구성
  ├── 9. 각 Feature.Contribute() → Mgmt-local/Biz-remote DesiredStateStore에 리소스 기여
  ├── 10. scan resource config allowlist 적용
  ├── 11. Mgmt Cluster 리소스 SSA Apply
  ├── 12. Biz Cluster Scan 실행
  │      ├── read-only applied config inspection
  │      └── 필요 시 허용된 scanner Job Remote SSA Apply
  ├── 13. label 기반 비활성 Feature/ScanRun remote GC
  ├── 14. Code 결과와 Biz Cluster 결과 상관 분석
  ├── 15. Evidence Bundle과 Exception Review 후보 생성
  └── 16. ClusterTarget.status와 ScanRun.status 갱신
```
