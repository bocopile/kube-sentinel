...
| Failure isolation | API 오류/timeout/quota/검증 실패는 전체 scan 실패가 아니다. `scan_health`에 degraded로 기록하고 정적 catalog로 fallback한다. |
...
| API 오류/timeout/quota | scan 계속, `scan_health` degraded(`ai_advisor_unavailable`), 정적 catalog 유지 |
...
| ID | 요구사항 | 검증 방법 |
| --- | --- | --- |
| G20 | AI remediation advisor는 기본 OFF opt-in이며, ON 시 advisory sidecar, provenance, redaction, `scan_health` degraded 기록을 제공한다. AI 실패는 scan Fail이 아니다. | AI ON/OFF scan에서 sidecar/provenance 생성, redaction fixture, Gemini 실패 시 scan Completed 확인 |
