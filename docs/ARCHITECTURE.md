...
| AI remediation model | Google AI Studio Gemini `gemini-2.5-flash` | Optional default-OFF provider for `remediation_enrichment` (see AI_REMEDIATION.md). Model id/version and prompt template hash are recorded as scanner-version-equivalent provenance; a model change updates this baseline row or is recorded as an evidence-bundle exception. |
...
15. Code / Artifact Scan과 Biz Cluster Scan 결과를 상관 분석하고 final decision을 확정한다.
    final decision 확정 직후, `report_export`(priority 300)로 넘어가기 전에 priority 250 post-decision
    enrichment feature(현재 `remediation_enrichment`, 상세는 [AI_REMEDIATION.md](./AI_REMEDIATION.md))의
    `Normalize()`를 별도로 실행한다 — 이 feature는 `findings.remediation`, severity, exception state,
    final decision을 변경하지 않고 advisory sidecar·provenance·scan_health만 추가로 기록할 수 있다.
...
- Mgmt Cluster RBAC for kube-sentinel CRDs, target kubeconfig Secrets, AI provider API key Secrets
  referenced by `spec.aiRemediation.apiKeySecretRef` (optional, see AI_REMEDIATION.md), status,
