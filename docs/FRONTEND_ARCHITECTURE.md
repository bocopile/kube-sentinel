...
로그인과 권한(정본은 [ARCHITECTURE.md](./ARCHITECTURE.md) §Dashboard/API backend and auth boundary):
...
| Admin | user/role mapping, backend auth 설정, retention 관리 |

UI는 사용자 역할에 따라 action button을 숨기거나 비활성화한다.
backend API도 같은 권한을 서버에서 다시 검증해야 하며, UI/API 모두 kubeconfig Secret value와 raw
Secret value를 표시하지 않는다.
...
| API | 역할 |
|-----|------|
| `auth` (backend 내 middleware) | login/session 검증과 user role resolution — 별도 서비스가 아니라 backend 요청 경로에 선행하는 middleware |
| `assessment-api` | scan run, finding, exception, 최종 판정 결과 조회와 scan/exception action 요청(역할별 제한) |
...
- 권한이 부족한 사용자는 scan 실행, retry/resume, exception approval action을 볼 수 없거나 실행할 수 없다.
